...
public:
   ...
   iterator erase(iterator const it) {
       if (it.act == nullptr) {
           throw std::out_of_range("El iterador no apunta a nada");
       }
       ...
       iterator retorno = it;
       retorno.next();
       if (this->array[indice] == actual) {
           this->array[indice] = actual->sig;
       }
       else {
           Link anterior = this->array[indice];
           while (anterior->sig != actual) anterior = anterior->sig;
